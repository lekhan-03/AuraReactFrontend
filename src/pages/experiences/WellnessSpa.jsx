...
import { Clock, MapPin, Sparkles, Check, Heart, Calendar } from 'lucide-react';

export default function WellnessSpa() {
  const data = experiencesData.wellness;
  ...
  const handleConfirmSession = (e) => {
    e.preventDefault();
    success(`Your reservation for "${bookingItem.title}" on ${sessionDate} (${preferredTime}) has been requested with your personal wellness curator.`);
    setBookingItem(null);
  };
  ...
  return (
    <div>
          ...
          <div key={item.id} className="experience-item-card">
            ...
            <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', flex: 1 }}>
              ...
              <div style={{ background: 'var(--bg-tertiary)', padding: '0.85rem 1rem', borderRadius: 'var(--radius-sm)', marginBottom: '1.5rem', fontSize: '0.82rem' }}>
                <strong style={{ display: 'block', marginBottom: '0.4rem', color: 'var(--text-primary)' }}>Therapy Highlights:</strong>
      ...
      {/* Booking Modal */}
      <Modal isOpen={!!bookingItem} onClose={() => setBookingItem(null)} title="Reserve Holistic Therapy">
        {bookingItem && (
          <form onSubmit={handleConfirmSession} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            <div>
            ...
            <div className="form-field-group">
              ...
              <select value={preferredTime} onChange={(e) => setPreferredTime(e.target.value)}>
                <option value="Sunrise Mindfulness (07:00 - 09:00)">Sunrise Mindfulness (07:00 - 09:00)</option>
                <option value="Morning (09:00 - 12:00)">Morning (09:00 - 12:00)</option>
                <option value="Afternoon (14:00 - 17:00)">Afternoon (14:00 - 17:00)</option>
                <option value="Sunset Serenity (17:30 - 19:30)">Sunset Serenity (17:30 - 19:30)</option>
              </select>
            </div>

            <button type="submit" className="btn btn-primary" style={{ marginTop: '0.5rem' }}>
              Confirm Wellness Reservation
            </button>
          </form>
        )}
      </Modal>
